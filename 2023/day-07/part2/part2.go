...
func NewHand(s string) Hand {
	...
	withoutJokers := WithoutJokers(parts[0])
	numJokers := CountJokers(parts[0])
	return Hand{
		ImmutableMultiSet: BestHand(withoutJokers, numJokers),
		Cards:             []Card(parts[0]),
		Bid:               bid,
	}
}

func CountJokers(s string) int {
	return strings.Count(s, "J")
}

func WithoutJokers(s string) *aocutil.ImmutableMultiSet[Card] {
	return aocutil.NewImmutableMultiSet([]Card(strings.ReplaceAll(s, "J", ""))...)
}

func BestHand(cardSet *aocutil.ImmutableMultiSet[Card], numJokers int) *aocutil.ImmutableMultiSet[Card] {
	if numJokers == 0 {
		return cardSet
	}
	if cardSet.Dimension() == 0 {
		return aocutil.NewImmutableMultiSet([]Card(strings.Repeat("J", numJokers))...)
	}

	mostFrequentCard := Card(-1)
	highestFrequency := 0
	for _, card := range cardSet.Values() {
		if card.Multiplicity > highestFrequency {
			mostFrequentCard = card.Val
			highestFrequency = card.Multiplicity
		}
	}
	return cardSet.With([]Card(strings.Repeat(fmt.Sprintf("%c", mostFrequentCard), numJokers))...)
}

type Card rune

type CardSet *aocutil.ImmutableMultiSet[Card]
